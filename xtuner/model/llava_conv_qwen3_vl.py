...
def _strip_image_tokens_vl(input_ids: torch.Tensor, labels: torch.Tensor, 
    ...
    This allows using full-modal data in text-only or text+WSI modes for Qwen3-VL.
    """
    B, L = input_ids.shape
    device = input_ids.device
    
    mask = input_ids != IMAGE_TOKEN_INDEX
    new_lengths = mask.sum(dim=1)
    max_new_len = new_lengths.max().item()
    
    if max_new_len == L:
        return input_ids, labels, attention_mask
    
    new_input_ids = torch.full((B, max_new_len), 0, dtype=input_ids.dtype, device=device)
    new_labels = torch.full((B, max_new_len), IGNORE_INDEX, dtype=labels.dtype, device=device) if labels is not None else None
    new_attention_mask = torch.zeros((B, max_new_len), dtype=attention_mask.dtype, device=device) if attention_mask is not None else None
    ...
    for b in range(B):
        ...
        if new_attention_mask is not None:
            new_attention_mask[b, :length] = attention_mask[b, valid_indices]
    
    return new_input_ids, new_labels, new_attention_mask


def _prepare_text_with_wsi_only(llm, input_ids, labels, attention_mask, wsi_embeddings, padding_side='right'):
    """
    ...
    Args:
        llm: Language model
        input_ids: (B, L) input token IDs (may contain <image> tokens to be stripped)
        labels: (B, L) labels for loss computation
        attention_mask: (B, L) attention mask
        wsi_embeddings: (B, Num_WSI_Sources, H) WSI global features
    ...
    if (input_ids == IMAGE_TOKEN_INDEX).any():
        input_ids, labels, attention_mask = _strip_image_tokens_vl(input_ids, labels, attention_mask)
    ...
    for batch_idx, cur_input_ids in enumerate(input_ids_list):
        ...
        # Prepend WSI embeddings
        cur_wsi_embeds = wsi_embeddings[batch_idx]  # (Num_WSI_Sources, H)
        ...
        # Position IDs: Linear for all tokens (3D format for Qwen3-VL compatibility)
        seq_len = combined_embeds.shape[0]
        pos_ids = torch.arange(seq_len, device=cur_input_ids.device).view(1, -1).expand(3, -1)
        
        new_inputs_embeds.append(combined_embeds)
        new_labels.append(combined_labels)
        new_visual_masks.append(combined_vmask)
    ...
    final_visual_pos_masks = torch.zeros((batch_size, max_len), dtype=torch.bool, device=device)
    final_position_ids = torch.zeros((3, batch_size, max_len), dtype=torch.long, device=device)
...
def prepare_inputs_labels_for_qwen3_vl(llm, input_ids, pixel_values, 
    ...
    Modes supported (can use same full-modal data, controlled by config only):
    1. Text-only: No pixel_values, no wsi_embeddings - pure text (strips <image> tokens if present)
    2. Text + WSI only: No pixel_values but has wsi_embeddings - WSI prepended (strips <image> tokens)
    3. Text + Patch: pixel_values but no wsi_embeddings - standard multimodal
    4. Full multimodal: Both pixel_values and wsi_embeddings
    ...
    # Pure text-only mode (no visual features of any kind)
    if not has_pixel_values and not has_wsi:
        # Strip <image> tokens if present (allows using full-modal data in text-only mode)
        if has_image_tokens:
            input_ids, labels, attention_mask = _strip_image_tokens_vl(input_ids, labels, attention_mask)
    ...
    # Text + WSI only mode (no patch-level visual features)
    # _prepare_text_with_wsi_only handles <image> token stripping internally
    if not has_pixel_values and has_wsi:
        return _prepare_text_with_wsi_only(
...
class LLaVAModel_conv(BaseModel):
    ...
    def forward(self, data: Dict[str, Any], data_samples: Optional[List] = None, mode: str = 'loss') -> Any:
        ...
        1. Text-only: No 'features' in data - pure text processing
        2. Text + WSI only: No 'features' but has 'wsi_features' - WSI features prepended
        3. Full multimodal: Has 'features' (and optionally 'wsi_features')
        """
        ...
        if has_visual_features:
            ...
            # Text-only or Text+WSI mode
            data['pixel_values'] = None
            data['deepstack_pixel_values'] = None
