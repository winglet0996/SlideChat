...
class LLaVAModel_longnet(BaseModel):

    def __init__(self,
                 llm, # removed visual_encoder
                 tokenizer,
                 freeze_llm=True, # lora
        ...
        self.visual_select_layer = visual_select_layer
         ...
         # 1. 构建 Tokenizer 实例，因为后面都需要用到
        self.tokenizer = BUILDER.build(tokenizer)

        # 2. 构建 GenerationConfig 对象
    ...
    def _project_vision_features(self, pixel_values):
        """Projects vision features using the LongNet encoder (if enabled)
        and the projector."""
